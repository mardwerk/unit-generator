...
const (
	draftStatusBudgetV2 = "Each tier has statChanges and statuses plus nullable detect/delivery/damageType/targeting fields. Null or an empty list means unchanged; do not repeat current values. Tier1/2 allow 1 to 3 primitive changes total; Tier3/4/5 allow up to 4, or tighter Definition limits. Count each statChanges or boostChanges entry and every nonnull enum/unlockBoost field as one. Each statuses entry counts as TWO changes when it has a magnitude and one otherwise, but one new capability. Tier1 and Tier2 may add only one new capability."
