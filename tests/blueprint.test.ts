...
test('optional interpretation survives numerical generation, save/reload, revision, checking and rendering', async () => {
  const input = plannedRequest();
  input.interpretation = interpretationFor(input);
  const model = new ResponseModel([{ output: plannedDesign() }, { output: blueprint() }]);
  const first = await draftUnit(await prepareRequest(input), model, { maxRepairAttempts: 0 });
  assert.deepEqual(first.run.designPlan!.interpretation, input.interpretation);
  assert.equal(model.requests.length, 2);
  const directory = await mkdtemp(join(tmpdir(), 'interpretation-cycle-'));
  try {
    const path = join(directory, 'candidate.json');
    await writeFile(path, JSON.stringify(first));
    const loaded = draftArtifactSchema.parse(JSON.parse(await readFile(path, 'utf8')));
    const checked = await checkDraft(loaded);
    assert.deepEqual(
    ...
    assert.ok(
      checked.findings.some(
        (finding) => finding.rule === 'retained-interpretation' && finding.outcome === 'pass',
      ),
    );
    const reviewer = new ResponseModel([
      {
        output: {
          summary:
            'The supplied interpretation is retained; this mocked review establishes no fidelity or gameplay quality.',
          findings: [],
        },
      },
    ]);
    const result = await reviewDraft(checked, reviewer);
    await writeFile(path, JSON.stringify(result));
    const reloaded = resultSchema.parse(JSON.parse(await readFile(path, 'utf8')));
    assert.deepEqual(reloaded.run.draft.designPlan!.interpretation, input.interpretation);
    assert.match(renderArtifact(reloaded), /Mira Spark/);
    assert.match(renderArtifact(reloaded, { details: true }), /Concentrated impact/);
    assert.match(
      renderArtifact(reloaded, { details: true }),
      /semantic fidelity, specialization use or balance/,
    );
    const next = {
      ...input,
      previous: { resultId: reloaded.id, draft: reloaded.candidate, findings: reloaded.findings },
      feedback: 'Keep the pinned arrangement and clarify the base attack name.',
    };
    const plan = plannedDesign();
    plan.base.name = 'Mira aimed Spark';
    const revised = await draftUnit(
      await prepareRequest(next),
      new ResponseModel([{ output: plan }, { output: blueprint() }]),
      { maxRepairAttempts: 0 },
    );
    assert.deepEqual(revised.run.designPlan!.interpretation, input.interpretation);
    assert.deepEqual(
      (await checkDraft(revised)).findings.filter((finding) => finding.outcome === 'fail'),
      [],
    );
    assert.match(renderArtifact(JSON.parse(JSON.stringify(revised))), /Mira aimed Spark/);

    const corrupted = structuredClone(revised);
    corrupted.run.designPlan!.paths.path1.sourceIds = [authorEvidence(next)[1]!.id];
    assert.ok(
      (await checkDraft(corrupted)).findings.some(
        (finding) =>
          finding.rule === 'interpretation-citation-coverage' && finding.outcome === 'fail',
      ),
    );
    const rebound = structuredClone(revised);
    rebound.run.designPlan!.interpretation!.reference.concepts[1]!.description =
      'A different caller interpretation.';
    assert.ok(
      (await checkDraft(rebound)).findings.some(
        (finding) => finding.rule === 'retained-interpretation' && finding.outcome === 'fail',
      ),
    );
  } finally {
