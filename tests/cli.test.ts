...
async function setup(body: (directory: string, requestPath: string) => Promise<void>) {
  ...
  try {
    ...
    const input = {
      ...request,
      documents: request.documents.map(({ origin: _origin, ...document }) => document),
    };
...
test('CLI stages serialize and reload; combined authoring and explicit revisions share the same core', async () => {
  await setup(async (directory, request) => {
    const stages = [
      ['prepare', request, 'prepared.json'],
      ['draft', 'prepared.json', 'draft.json'],
      ['check', 'draft.json', 'checked.json'],
      ['review', 'checked.json', 'result.json'],
    ] as const;
    for (const [command, input, output] of stages) {
      const response = await run(
        [command, input, '--output', output, '--codex', './fake-codex.mjs'],
        directory,
      );
    ...
    const revised = await run(
      [
        ...
        'result.json',
        '--feedback',
        'Prioritize support in this revision',
        '--codex',
        './fake-codex.mjs',
      ],
      directory,
...
test('fake CLI model calls use fixture configuration even when the caller configuration is malformed', async () => {
  await setup(async (directory, request) => {
    ...
    const response = await run(['author', request, '--codex', './fake-codex.mjs'], directory, {
      ...process.env,
...
test('CLI failures leave stdout and intended output empty and never overwrite existing work', async () => {
  await setup(async (directory, request) => {
    ...
    const existing = await run(
      ['author', request, '-o', output, '--codex', '/does-not-exist'],
      directory,
    );
