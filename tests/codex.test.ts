...
async function fakeCodex(
  body: string,
  run: (executable: string, directory: string) => Promise<void>,
) {
...
test('Codex requests are isolated, stdin-fed, tool-limited and parsed from the final file only', async () => {
  await fakeCodex(
    ...
    fs.writeFileSync(output, JSON.stringify({ args, prompt, schema, files: fs.readdirSync(process.cwd()), home: process.env.HOME, codexHome: process.env.CODEX_HOME ?? null }));`,
...
test('Codex failures do not reveal provider console secrets and always clean temp files', async () => {
  await fakeCodex(
    `console.error('SECRET_CREDENTIAL_TOKEN'); process.exit(7);`,
    async (executable, directory) => {
...
test('Codex timeout, cancellation and output limits terminate the process and clean up', async () => {
  await fakeCodex(`setInterval(() => {}, 1000);`, async (executable, directory) => {
    ...
    const pending = new CodexModelClient({
      executable,
      configPath: join(directory, 'config.toml'),
      timeoutMs: 5000,
    }).generate({
...
test('Codex preserves configured provider and auth while disabling every configured MCP server', async () => {
  await fakeCodex(
    `for await (const chunk of process.stdin) {} fs.writeFileSync(output, JSON.stringify({ args }));`,
    async (executable, directory) => {
      ...
      assert.ok(
        result.args.includes(
          'mcp_servers={"docs"={enabled=false},"dotted.server"={enabled=false}}',
        ),
      );
      assert.ok(!result.args.some((arg) => arg.startsWith('model_reasoning_effort=')));
      assert.doesNotMatch(
        JSON.stringify(result),
        /SECRET_TOKEN_VALUE|SECRET_COMMAND_VALUE|private-provider|private\.invalid/,
      );
      const override = new CodexModelClient({
        executable,
        configPath,
