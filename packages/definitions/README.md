...
`classic-three-path` is the editable BTD6-inspired default. `merge-family-example` demonstrates a different input, output, and set of checks. `manga-mayhem` and `btd6-derived` are separately versioned candidate game contracts with executable checks and declared simulation limits. Each directory under `definitions/` contains the manifest, rules, generation instructions, schemas, and optional examples. Model transport and storage do not belong in a definition.
...
The candidate contracts export their own schemas, compilers, fixtures and deterministic encounter probes from `@mardwerk/unit-definitions/manga-mayhem` and `@mardwerk/unit-definitions/btd6-derived`. Their checks establish their declared supported scope, not full game-engine fidelity. `createBundledFixture` supplies explicitly synthetic original-concept demonstrations to both the CLI and playground.
