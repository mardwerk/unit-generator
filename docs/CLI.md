...
This is today's TypeScript CLI. The planned Go CLI (`mardwerk-unit`) is described in [ARCHITECTURE.md](ARCHITECTURE.md#cli); command names change there (`character` becomes `research`, `--preset btd6` becomes `--profile`).
...
pnpm cli prepare REQUEST.json --preset btd6 -o data/runs/prepared.json
...
Model inputs and raw outputs are recorded only with `--evidence-dir`.
