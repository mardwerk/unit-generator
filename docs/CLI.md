...
Run `pnpm install` and `pnpm build` with Node.js 24 or newer. `pnpm cli --help` lists options. Drafting with the default `universal-v1` route needs no key and makes no model calls. Model review and `--authoring planned-v1` use the OpenRouter SDK with `openrouter/free` and require `OPENROUTER_API_KEY`. Use `--provider codex` for the existing local Codex configuration and login. `--model`, `--reasoning` and `--timeout` override that call. OpenRouter reasoning defaults to `none`; `low`, `medium` and `high` are optional. Codex retains its own configured default and accepts those three levels. Default timeouts are 120 seconds for OpenRouter and 600 seconds for Codex. Large source and rules documents can take several minutes to process.
...
`generate` returns a checked artifact using the explicit [BTD6-inspired mechanics definition](MECHANICS.md). The default `universal-v1` route derives traits from the evidence, builds the best-fit tested recipe, runs a bounded code-only search, and resolves all 64 legal builds. No model call runs. `--authoring planned-v1|direct|reference-patterns-v1` selects another draft route for `character`, `generate`, `prepare`, `author` or `draft`; the model routes need a configured provider. On invalid design output, one repair receives the failed checks. `--repairs 0` disables this; `--repairs 2` permits two repairs. Authentication, rate limits and timeouts stop immediately. Invalid blueprints never become published candidates. This is mechanics validation, not combat simulation or balance approval. Independent semantic `review` remains a separate call so a failed review cannot discard the saved Unit.
