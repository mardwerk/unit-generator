...
- The unit sheet shows references on the left, the unit in the middle, and stages and usage on the right. Each path is labeled top, middle or bottom and each purchase by build code (`1-x-x`, `x-4-x`, `x-x-5`); opening a purchase shows its exact effects. Unsupported mechanics are listed below the paths, **Crosspaths** lists every legal two-path build (12 early and 36 advanced under the default) with what each path adds and the resulting attack, and a revision gets **Revision changes** with changed mechanics apart from renamed purchases. Failed checks are listed below the sheet.
