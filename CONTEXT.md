...
| Active Ability | An ability that the player activates by clicking it. In a blueprint, owned `abilities` are Active Abilities; automatic effects are stats, statuses and follow-ups, not abilities. The design policy calls an Active Ability a manual ability (`manualAbilityPath`, `maxManualAbilityPaths`). Those field names stay, and they refer only to Active Abilities. |
