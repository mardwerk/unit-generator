...
| Profile | A reusable, editable file that selects a Definition and the values it allows, plus rules text and the task. The bundled default is read-only; saved Profiles are copies. |
...
| Library | The folder, chosen by the user, where saved artifacts, icons, portraits and Profiles live. The CLI and `serve` write it; nothing else is stored. |
...
| --- | --- | --- |
| Unit Generator | Unit research, preparation, generation, checks, review, rendering, its library folder | Project history, orchestration across tools, runtime behavior |
